...
settings.addInput('progress_throttle', 'How requent the progress data is send (ms)', '900', () => progress_throttle = Number(settings.getFieldValue('progress_throttle')));
...
async function on_progress(data: any) {
    ...
    socket?.emit('progress', { progress_ms: Spicetify.Player.getProgress(), progress_p: (Spicetify.Player.getProgressPercent() * 100)})
}
